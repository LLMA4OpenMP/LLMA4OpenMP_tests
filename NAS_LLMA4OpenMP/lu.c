...
#pragma omp parallel for private(j, m)
	for (i = iend; i >= ist; i--)
	{
		for (j = jend; j >= jst; j--)
		{
			...
			{
				tv[i][j][m] =
					omega * (udz[i][j][m][0] * v[i][j][k + 1][0] + udz[i][j][m][1] * v[i][j][k + 1][1] + udz[i][j][m][2] * v[i][j][k + 1][2] + udz[i][j][m][3] * v[i][j][k + 1][3] + udz[i][j][m][4] * v[i][j][k + 1][4]);
			}
		}
	}
...
#pragma omp parallel for private(i, iglob, xi, j, jglob, eta, k, zeta, m)
		for (i = 0; i < nx; i++)
		{
			iglob = i;
...
#pragma omp parallel for private(i, k, m, tmp, u21i, u31i, u41i, u51i, u21im1, u31im1, u41im1, u51im1)
		for (j = jst; j <= jend; j++)
		{
			for (k = 1; k <= nz - 2; k++)
			{
...
#pragma omp parallel for private(j, k, m, tmp, u21j, u31j, u41j, u51j, u21jm1, u31jm1, u41jm1, u51jm1)
		for (i = ist; i <= iend; i++)
		{
			for (k = 1; k <= nz - 2; k++)
			{
...
#pragma omp parallel for private(j, k, m, u41, q, tmp, u21k, u31k, u41k, u51k, u21km1, u31km1, u41km1, u51km1)
		for (i = ist; i <= iend; i++)
		{
			for (j = jst; j <= jend; j++)
			{
...
static void jacld(int k)
{
	...
	c1345 = C1 * C3 * C4 * C5;
	c34 = C3 * C4;
#pragma omp parallel for private(j, tmp1, tmp2, tmp3)
	for (i = ist; i <= iend; i++)
	{
		for (j = jst; j <= jend; j++)
		{
...
#pragma omp parallel for private(i, j, iglob, jglob, k)
	for (i = ibeg; i <= ifin; i++)
	{
		iglob = i;
		for (j = jbeg; j <= jfin; j++)
...
#pragma omp parallel for private(k, i, m, L2, tmp, u21i, u31i, u41i, u51i, u21im1, u31im1, u41im1, u51im1, ist1, iend1)
		for (j = jst; j <= jend; j++)
		{
			for (k = 1; k <= nz - 2; k++)
			{
