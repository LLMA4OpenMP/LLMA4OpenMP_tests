...
int main(int argc, char **argv)
{
  ...
  if (tmax != 0)
  {
    mflops = (881.174 * pow((double)PROBLEM_SIZE, 3.0) - 4683.91 * pow2((double)PROBLEM_SIZE) + 11484.5 * (double)PROBLEM_SIZE - 19272.4) * (double)niter / (tmax * 1000000.0);
  }
  else
  {
    mflops = 0.0;
...
static void error_norm(double rms[5])
{
  ...
  for (m = 0; m < 5; m++)
  {
    for (d = 0; d < 3; d++)
    {
      // This loop should not be parallelized because:
      // The loop only iterates 3 times, which is also less than the threshold of 50 iterations.
      // Parallelizing loops with too few iterations is not efficient and can lead to performance issues.
...
static void rhs_norm(double rms[5])
{
  ...
  {
    rms[m] = 0.0;
    // Loop not parallelized due to too few iterations (less than 50)
  }

#pragma omp parallel for private(j, k, m) reduction(+ : rms[ : 5])
...
#pragma omp parallel for private(i, im1, ip1)
      for (i = 1; i <= grid_points[0] - 2; i++)
      {
        ...
        forcing[1][i][j][k] = forcing[1][i][j][k] - tx2 * ((ue[1][ip1] * buf[1][ip1] + c2 * (ue[4][ip1] - q[ip1])) - (ue[1][im1] * buf[1][im1] + c2 * (ue[4][im1] - q[im1]))) +
                              xxcon1 * (buf[1][ip1] - 2.0 * buf[1][i] + buf[1][im1]) +
                              dx2tx1 * (ue[1][ip1] - 2.0 * ue[1][i] + ue[1][im1]);

        forcing[2][i][j][k] = forcing[2][i][j][k] - tx2 * (ue[2][ip1] * buf[1][ip1] - ue[2][im1] * buf[1][im1]) +
                              xxcon2 * (buf[2][ip1] - 2.0 * buf[2][i] + buf[2][im1]) +
                              dx3tx1 * (ue[2][ip1] - 2.0 * ue[2][i] + ue[2][im1]);

        forcing[3][i][j][k] = forcing[3][i][j][k] - tx2 * (ue[3][ip1] * buf[1][ip1] - ue[3][im1] * buf[1][im1]) +
                              xxcon2 * (buf[3][ip1] - 2.0 * buf[3][i] + buf[3][im1]) +
                              dx4tx1 * (ue[3][ip1] - 2.0 * ue[3][i] + ue[3][im1]);

        forcing[4][i][j][k] = forcing[4][i][j][k] - tx2 * (buf[1][ip1] * (c1 * ue[4][ip1] - c2 * q[ip1]) - buf[1][im1] * (c1 * ue[4][im1] - c2 * q[im1])) +
...
#pragma omp parallel for private(jm1, jp1)
      for (j = 1; j <= grid_points[1] - 2; j++)
      {
        jm1 = j - 1;
        jp1 = j + 1;
        ...
        forcing[1][i][j][k] = forcing[1][i][j][k] - ty2 * (ue[1][jp1] * buf[2][jp1] - ue[1][jm1] * buf[2][jm1]) +
                              yycon2 * (buf[1][jp1] - 2.0 * buf[1][j] + buf[1][jm1]) +
                              dy2ty1 * (ue[1][jp1] - 2.0 * ue[1][j] + ue[1][jm1]);

        forcing[2][i][j][k] = forcing[2][i][j][k] - ty2 * ((ue[2][jp1] * buf[2][jp1] + c2 * (ue[4][jp1] - q[jp1])) - (ue[2][jm1] * buf[2][jm1] + c2 * (ue[4][jm1] - q[jm1]))) +
                              yycon1 * (buf[2][jp1] - 2.0 * buf[2][j] + buf[2][jm1]) +
                              dy3ty1 * (ue[2][jp1] - 2.0 * ue[2][j] + ue[2][jm1]);

        forcing[3][i][j][k] = forcing[3][i][j][k] - ty2 * (ue[3][jp1] * buf[2][jp1] - ue[3][jm1] * buf[2][jm1]) +
                              yycon2 * (buf[3][jp1] - 2.0 * buf[3][j] + buf[3][jm1]) +
                              dy4ty1 * (ue[3][jp1] - 2.0 * ue[3][j] + ue[3][jm1]);

        forcing[4][i][j][k] = forcing[4][i][j][k] - ty2 * (buf[2][jp1] * (c1 * ue[4][jp1] - c2 * q[jp1]) - buf[2][jm1] * (c1 * ue[4][jm1] - c2 * q[jm1])) +
...
#pragma omp parallel for private(km1, kp1)
      for (k = 1; k <= grid_points[2] - 2; k++)
      {
        km1 = k - 1;
        kp1 = k + 1;
        ...
        forcing[1][i][j][k] = forcing[1][i][j][k] - tz2 * (ue[1][kp1] * buf[3][kp1] - ue[1][km1] * buf[3][km1]) +
                              zzcon2 * (buf[1][kp1] - 2.0 * buf[1][k] + buf[1][km1]) +
                              dz2tz1 * (ue[1][kp1] - 2.0 * ue[1][k] + ue[1][km1]);

        forcing[2][i][j][k] = forcing[2][i][j][k] - tz2 * (ue[2][kp1] * buf[3][kp1] - ue[2][km1] * buf[3][km1]) +
                              zzcon2 * (buf[2][kp1] - 2.0 * buf[2][k] + buf[2][km1]) +
                              dz3tz1 * (ue[2][kp1] - 2.0 * ue[2][k] + ue[2][km1]);

        forcing[3][i][j][k] = forcing[3][i][j][k] - tz2 * ((ue[3][kp1] * buf[3][kp1] + c2 * (ue[4][kp1] - q[kp1])) - (ue[3][km1] * buf[3][km1] + c2 * (ue[4][km1] - q[km1]))) +
                              zzcon1 * (buf[3][kp1] - 2.0 * buf[3][k] + buf[3][km1]) +
                              dz4tz1 * (ue[3][kp1] - 2.0 * ue[3][k] + ue[3][km1]);

        forcing[4][i][j][k] = forcing[4][i][j][k] - tz2 * (buf[3][kp1] * (c1 * ue[4][kp1] - c2 * q[kp1]) - buf[3][km1] * (c1 * ue[4][km1] - c2 * q[km1])) +
...
static void pinvr(void)
{
  ...
  int i, j, k;
  double r1, r2, r3, r4, r5, t1, t2;
#pragma omp parallel for private(j, k, r1, r2, r3, r4, r5, t1, t2)
  for (i = 1; i <= grid_points[0] - 2; i++)
  {
    for (j = 1; j <= grid_points[1] - 2; j++)
    {
    ...
    for (i = 1; i <= grid_points[0] - 2; i++)
    {
      for (j = 1; j <= grid_points[1] - 2; j++)
      {
        for (k = 1; k <= grid_points[2] - 2; k++)
        {
          ...
          rhs[0][i][j][k] = rhs[0][i][j][k] + dx1tx1 * (u[0][i + 1][j][k] - 2.0 * u[0][i][j][k] + u[0][i - 1][j][k]) -
                            tx2 * (u[1][i + 1][j][k] - u[1][i - 1][j][k]);
          rhs[1][i][j][k] = rhs[1][i][j][k] + dx2tx1 * (u[1][i + 1][j][k] - 2.0 * u[1][i][j][k] + u[1][i - 1][j][k]) +
          ...
          rhs[2][i][j][k] = rhs[2][i][j][k] + dx3tx1 * (u[2][i + 1][j][k] - 2.0 * u[2][i][j][k] + u[2][i - 1][j][k]) +
                            xxcon2 * (vs[i + 1][j][k] - 2.0 * vs[i][j][k] +
                                      vs[i - 1][j][k]) -
                            tx2 * (u[2][i + 1][j][k] * up1 -
                                   u[2][i - 1][j][k] * um1);

          rhs[3][i][j][k] = rhs[3][i][j][k] + dx4tx1 * (u[3][i + 1][j][k] - 2.0 * u[3][i][j][k] + u[3][i - 1][j][k]) +
                            xxcon2 * (ws[i + 1][j][k] - 2.0 * ws[i][j][k] +
                                      ws[i - 1][j][k]) -
                            tx2 * (u[3][i + 1][j][k] * up1 -
                                   u[3][i - 1][j][k] * um1);

          rhs[4][i][j][k] = rhs[4][i][j][k] + dx5tx1 * (u[4][i + 1][j][k] - 2.0 * u[4][i][j][k] + u[4][i - 1][j][k]) +
...
#pragma omp parallel for private(j, k, vijk, vp1, vm1)
    for (i = 1; i <= grid_points[0] - 2; i++)
    {
      for (j = 1; j <= grid_points[1] - 2; j++)
      {
        for (k = 1; k <= grid_points[2] - 2; k++)
        {
          ...
          vp1 = vs[i][j + 1][k];
          vm1 = vs[i][j - 1][k];
          rhs[0][i][j][k] = rhs[0][i][j][k] + dy1ty1 * (u[0][i][j + 1][k] - 2.0 * u[0][i][j][k] + u[0][i][j - 1][k]) -
                            ty2 * (u[2][i][j + 1][k] - u[2][i][j - 1][k]);
          rhs[1][i][j][k] = rhs[1][i][j][k] + dy2ty1 * (u[1][i][j + 1][k] - 2.0 * u[1][i][j][k] + u[1][i][j - 1][k]) +
                            yycon2 * (us[i][j + 1][k] - 2.0 * us[i][j][k] +
                                      us[i][j - 1][k]) -
                            ty2 * (u[1][i][j + 1][k] * vp1 -
                                   u[1][i][j - 1][k] * vm1);
          rhs[2][i][j][k] = rhs[2][i][j][k] + dy3ty1 * (u[2][i][j + 1][k] - 2.0 * u[2][i][j][k] + u[2][i][j - 1][k]) +
                            ...
                            ty2 * (u[2][i][j + 1][k] * vp1 -
                                   ...
                                   (u[4][i][j + 1][k] - square[i][j + 1][k] -
                                    u[4][i][j - 1][k] + square[i][j - 1][k]) *
                                       c2);
          rhs[3][i][j][k] = rhs[3][i][j][k] + dy4ty1 * (u[3][i][j + 1][k] - 2.0 * u[3][i][j][k] + u[3][i][j - 1][k]) +
                            yycon2 * (ws[i][j + 1][k] - 2.0 * ws[i][j][k] +
                                      ws[i][j - 1][k]) -
                            ty2 * (u[3][i][j + 1][k] * vp1 -
                                   u[3][i][j - 1][k] * vm1);
          rhs[4][i][j][k] = rhs[4][i][j][k] + dy5ty1 * (u[4][i][j + 1][k] - 2.0 * u[4][i][j][k] + u[4][i][j - 1][k]) +
...
#pragma omp parallel for private(j, k, wijk, wp1, wm1)
    for (i = 1; i <= grid_points[0] - 2; i++)
    {
      for (j = 1; j <= grid_points[1] - 2; j++)
      {
        for (k = 1; k <= grid_points[2] - 2; k++)
        {
          ...
          rhs[0][i][j][k] = rhs[0][i][j][k] + dz1tz1 * (u[0][i][j][k + 1] - 2.0 * u[0][i][j][k] + u[0][i][j][k - 1]) -
                            tz2 * (u[3][i][j][k + 1] - u[3][i][j][k - 1]);
          rhs[1][i][j][k] = rhs[1][i][j][k] + dz2tz1 * (u[1][i][j][k + 1] - 2.0 * u[1][i][j][k] + u[1][i][j][k - 1]) +
                            zzcon2 * (us[i][j][k + 1] - 2.0 * us[i][j][k] +
                                      us[i][j][k - 1]) -
                            tz2 * (u[1][i][j][k + 1] * wp1 -
                                   u[1][i][j][k - 1] * wm1);
          rhs[2][i][j][k] = rhs[2][i][j][k] + dz3tz1 * (u[2][i][j][k + 1] - 2.0 * u[2][i][j][k] + u[2][i][j][k - 1]) +
                            zzcon2 * (vs[i][j][k + 1] - 2.0 * vs[i][j][k] +
                                      vs[i][j][k - 1]) -
                            tz2 * (u[2][i][j][k + 1] * wp1 -
                                   u[2][i][j][k - 1] * wm1);
          rhs[3][i][j][k] = rhs[3][i][j][k] + dz4tz1 * (u[3][i][j][k + 1] - 2.0 * u[3][i][j][k] + u[3][i][j][k - 1]) +
                            ...
                            tz2 * (u[3][i][j][k + 1] * wp1 -
                                   ...
                                   (u[4][i][j][k + 1] - square[i][j][k + 1] -
                                    u[4][i][j][k - 1] + square[i][j][k - 1]) *
                                       c2);
          rhs[4][i][j][k] = rhs[4][i][j][k] + dz5tz1 * (u[4][i][j][k + 1] - 2.0 * u[4][i][j][k] + u[4][i][j][k - 1]) +
...
static void txinvr(void)
{
  ...
  double t1, t2, t3, ac, ru1, uu, vv, ww, r1, r2, r3,
      r4, r5, ac2inv;
#pragma omp parallel for private(j, k, ru1, uu, vv, ww, ac, ac2inv, r1, r2, r3, r4, r5, t1, t2, t3)
  for (i = 1; i <= grid_points[0] - 2; i++)
  {
    for (j = 1; j <= grid_points[1] - 2; j++)
    {
...
static void x_solve(void)
{

  {
    ...
    for (i = 0; i <= grid_points[0] - 3; i++)
    {
      i1 = i + 1;
      i2 = i + 2;
#pragma omp parallel for private(k, m, fac1)
      for (j = 1; j <= grid_points[1] - 2; j++)
      {
        for (k = 1; k <= grid_points[2] - 2; k++)
        {
    ...
    i = grid_points[0] - 2;
    i1 = grid_points[0] - 1;
#pragma omp parallel for private(k, m, fac1, fac2)
    for (j = 1; j <= grid_points[1] - 2; j++)
    {
      for (k = 1; k <= grid_points[2] - 2; k++)
      {
    ...
    for (m = 3; m < 5; m++)
    {
      ...
      for (i = 0; i <= grid_points[0] - 3; i++)
      {
        i1 = i + 1;
        i2 = i + 2;
#pragma omp parallel for private(k, fac1)
        for (j = 1; j <= grid_points[1] - 2; j++)
        {
          for (k = 1; k <= grid_points[2] - 2; k++)
          {
...
static void y_solve(void)
{

  {
    ...
    for (j = 0; j <= grid_points[1] - 3; j++)
    {
      j1 = j + 1;
      j2 = j + 2;
#pragma omp parallel for private(i, k, m, fac1)
      for (i = 1; i <= grid_points[0] - 2; i++)
      {
        for (k = 1; k <= grid_points[2] - 2; k++)
        {
...
#pragma omp parallel for private(i, k, m, fac1, fac2)
    ...
    for (m = 3; m < 5; m++)
    {
      ...
      for (j = 0; j <= grid_points[1] - 3; j++)
      {
        j1 = j + 1;
        j2 = j + 2;
#pragma omp parallel for private(i, k, fac1)
        for (i = 1; i <= grid_points[0] - 2; i++)
        {
          for (k = 1; k <= grid_points[2] - 2; k++)
          {
...
static void z_solve(void)
{

  {
    ...
    k = grid_points[2] - 2;
    k1 = grid_points[2] - 1;
#pragma omp parallel for private(fac1, fac2, m, j)
    for (i = 1; i <= grid_points[0] - 2; i++)
    {
      for (j = 1; j <= grid_points[1] - 2; j++)
      {
