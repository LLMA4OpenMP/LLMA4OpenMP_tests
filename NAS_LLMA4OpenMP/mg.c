				  ...
				  double c[4], int k)
{
	...
	int i3, i2, i1;
	double r1[M], r2[M];
#pragma omp parallel for private(i2, i1, r1, r2)
	for (i3 = 1; i3 < n3 - 1; i3++)
	{
		for (i2 = 1; i2 < n2 - 1; i2++)
		{
				  ...
				  int n1, int n2, int n3, double a[4], int k)
{
	...
	int i3, i2, i1;
	double u1[M], u2[M];
#pragma omp parallel for private(i2, i1, u1, u2)
	for (i3 = 1; i3 < n3 - 1; i3++)
	{
		for (i2 = 1; i2 < n2 - 1; i2++)
		{
...
#pragma omp parallel for private(i1, y2, x2)
...
#pragma omp parallel for
	...
	for (i = MM - 1; i >= 0; i--)
	{
		// The value of 'best' depends on 'i1' which is modified within the loop.
		...
		if (best == z[j3[i1][1]][j2[i1][1]][j1[i1][1]])
		{
			...
			i1 = i1 - 1; // Modifying 'i1' here affects the next iteration, creating a dependency.
		}
		else
		{
			jg[0][i][1] = 0;
		...
		// The value of 'best' depends on 'i0' which is modified within the loop.
		// This creates a dependency between iterations.
		...
		if (best == z[j3[i0][0]][j2[i0][0]][j1[i0][0]])
		{
			...
			i0 = i0 - 1; // Modifying 'i0' here affects the next iteration, creating a dependency.
		}
		else
		{
			jg[0][i][0] = 0;
	...
	// This loop cannot be parallelized due to the dependencies on 'i1' and 'i0'.
	// Modifying 'i1' and 'i0' within the loop creates a data race condition.
	// Additionally, there is a sequential dependency because the iterations are not independent.
	m1 = i1 + 1;
	m0 = i0 + 1;
