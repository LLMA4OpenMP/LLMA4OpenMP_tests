...
int main(int argc, char **argv)
{
   ...
   if (tmax!=0)
      mflops = (881.174*pow((double) 102, 3.0)-4683.91*(double) 102*(double) 102+11484.5*(double) 102-19272.4)*(double) niter/(tmax*1000000.0);
   else
      mflops = 0.0;
   ...
   c_print_results("SP", class, grid_points[0], grid_points[1], grid_points[2], niter, nthreads, tmax, mflops, "          floating point", verified, "3.0 structured", "01 Dec 2023", "(none)", "(none)", "-lm", "(none)", "(none)", "(none)", "(none)");
}
static void add(void)
{
   int i, j, k, m;
...
#pragma omp parallel for private(ip1, im1)
         for(i = 1; i <= grid_points[0]-2; i += 1) {
            im1 = i-1;
            ip1 = i+1;
...
#pragma omp parallel for private(kp1, km1)
         for(k = 1; k <= grid_points[2]-2; k += 1) {
            km1 = k-1;
            kp1 = k+1;
...
#pragma omp parallel for private(j, k, t2, t1, r5, r4, r3, r2, r1)
   for(i = 1; i <= grid_points[0]-2; i += 1)
      for(j = 1; j <= grid_points[1]-2; j += 1)
         for(k = 1; k <= grid_points[2]-2; k += 1) {
...
#pragma omp parallel for private(j, k, rho_inv, aux)
   for(i = 0; i <= grid_points[0]-1; i += 1)
      for(j = 0; j <= grid_points[1]-1; j += 1)
         for(k = 0; k <= grid_points[2]-1; k += 1) {
...
#pragma omp parallel for private(i, j, k)
   ...
   c-------------------------------------------------------------------*/
#pragma omp parallel for private(j, k, up1, um1, uijk)
   for(i = 1; i <= grid_points[0]-2; i += 1)
      for(j = 1; j <= grid_points[1]-2; j += 1)
         for(k = 1; k <= grid_points[2]-2; k += 1) {
            uijk = us[i][j][k];
...
#pragma omp parallel for private(i, j, k)
   for(m = 0; m <= 4; m += 1)
      for(i = 3*1; i <= grid_points[0]-3*1-1; i += 1)
         for(j = 1; j <= grid_points[1]-2; j += 1)
            for(k = 1; k <= grid_points[2]-2; k += 1)
...
#pragma omp parallel for private(j, k)
   ...
   c-------------------------------------------------------------------*/
#pragma omp parallel for private(j, k, vp1, vm1, vijk)
   for(i = 1; i <= grid_points[0]-2; i += 1)
      for(j = 1; j <= grid_points[1]-2; j += 1)
         for(k = 1; k <= grid_points[2]-2; k += 1) {
            vijk = vs[i][j][k];
...
#pragma omp parallel for private(i, j, k)
   for(m = 0; m <= 4; m += 1)
      for(i = 1; i <= grid_points[0]-2; i += 1)
         for(j = 3*1; j <= grid_points[1]-3*1-1; j += 1)
            for(k = 1; k <= grid_points[2]-2; k += 1)
...
#pragma omp parallel for private(i, k)
   ...
   c-------------------------------------------------------------------*/
#pragma omp parallel for private(j, k, wp1, wm1, wijk)
   for(i = 1; i <= grid_points[0]-2; i += 1)
      for(j = 1; j <= grid_points[1]-2; j += 1)
         for(k = 1; k <= grid_points[2]-2; k += 1) {
            wijk = ws[i][j][k];
...
#pragma omp parallel for private(i, j, k)
   for(m = 0; m <= 4; m += 1)
      for(i = 1; i <= grid_points[0]-2; i += 1)
         for(j = 1; j <= grid_points[1]-2; j += 1)
            for(k = 3*1; k <= grid_points[2]-3*1-1; k += 1)
...
#pragma omp parallel for private(j, k, ww, vv, uu, t3, t2, t1, ru1, r5, r4, r3, r2, r1, ac2inv, ac)
   for(i = 1; i <= grid_points[0]-2; i += 1)
      for(j = 1; j <= grid_points[1]-2; j += 1)
         for(k = 1; k <= grid_points[2]-2; k += 1) {
...
#pragma omp parallel for private(j, k, zvel, yvel, xvel, uzik1, t3, t2, t1, r5, r4, r3, r2, r1, btuz, acinv, ac2u, ac)
   for(i = 1; i <= grid_points[0]-2; i += 1)
      for(j = 1; j <= grid_points[1]-2; j += 1)
         for(k = 1; k <= grid_points[2]-2; k += 1) {
...
#pragma omp parallel for private(k, m, fac1)
      ...
      i = grid_points[0]-2;
      i1 = grid_points[0]-1;
#pragma omp parallel for private(k, m, fac2, fac1)
      for(j = 1; j <= grid_points[1]-2; j += 1)
         for(k = 1; k <= grid_points[2]-2; k += 1) {
...
#pragma omp parallel for private(n, j, i2, i1, i, k, fac2, fac1)
      for(m = 3; m <= 4; m += 1) {
         n = (m-3+1)*5;
         for(i = 0; i <= grid_points[0]-3; i += 1) {
            i1 = i+1;
...
#pragma omp parallel for private(j, k)
      ...
      --------------------------------------------------------------------*/
#pragma omp parallel for private(n, j, i2, i1, i, k)
      for(m = 3; m <= 4; m += 1) {
         n = (m-3+1)*5;
         for(i = grid_points[0]-3; i >= 0; i += -1) {
            i1 = i+1;
...
#pragma omp parallel for private(k, m, fac2, fac1)
      ...
      c      do the u+c and the u-c factors                 
      --------------------------------------------------------------------*/
#pragma omp parallel for private(n, j2, j1, j, i, k, fac2, fac1)
      for(m = 3; m <= 4; m += 1) {
         n = (m-3+1)*5;
         for(j = 0; j <= grid_points[1]-3; j += 1) {
            j1 = j+1;
...
#pragma omp parallel for private(i, k, n)
      ...
      --------------------------------------------------------------------*/
      n = 0;
#pragma omp parallel for private(j2, j1, j, i, k)
      for(m = 0; m <= 2; m += 1)
      ...
      c      And the remaining two
      --------------------------------------------------------------------*/
#pragma omp parallel for private(n, j2, j1, j, i, k)
      for(m = 3; m <= 4; m += 1) {
         n = (m-3+1)*5;
         for(j = grid_points[1]-3; j >= 0; j += -1) {
            j1 = j+1;
...
#pragma omp parallel for private(j, m, k2, k1, k, fac1)
      for(i = 1; i <= grid_points[0]-2; i += 1)
         for(j = 1; j <= grid_points[1]-2; j += 1)
      ...
      k = grid_points[2]-2;
      k1 = grid_points[2]-1;
#pragma omp parallel for private(j, m, fac2, fac1)
      for(i = 1; i <= grid_points[0]-2; i += 1)
         for(j = 1; j <= grid_points[1]-2; j += 1) {
            fac1 = 1./lhs[n+2][i][j][k];
            lhs[n+3][i][j][k] = fac1*lhs[n+3][i][j][k];
      ...
      c      do the u+c and the u-c factors               
      c-------------------------------------------------------------------*/
#pragma omp parallel for private(n, k1, k, i, j, fac2, fac1, k2)
      for(m = 3; m <= 4; m += 1) {
         n = (m-3+1)*5;
         for(i = 1; i <= grid_points[0]-2; i += 1)
            for(j = 1; j <= grid_points[1]-2; j += 1)
...
#pragma omp parallel for private(n, i, j)
      ...
      c-------------------------------------------------------------------*/
      n = 0;
#pragma omp parallel for private(i, j, k2, k1, k)
      for(m = 0; m <= 2; m += 1)
         for(i = 1; i <= grid_points[0]-2; i += 1)
            for(j = 1; j <= grid_points[1]-2; j += 1)
      ...
      c      And the remaining two
      c-------------------------------------------------------------------*/
#pragma omp parallel for private(n, i, j, k2, k1, k)
      for(m = 3; m <= 4; m += 1) {
         n = (m-3+1)*5;
         for(i = 1; i <= grid_points[0]-2; i += 1)
            for(j = 1; j <= grid_points[1]-2; j += 1)
