...
int main(int argc, char **argv)
{
   ...
   c_print_results("BT", class, grid_points[0], grid_points[1], grid_points[2], niter, nthreads, tmax, mflops, "          floating point", verified, "3.0 structured", "17 Jan 2020", "(none)", "(none)", "-lm", "(none)", "(none)", "(none)", "(none)");
}
static void add(void)
{
...
#pragma omp parallel for private(i)
         for(m = 0; m <= 4; m += 1)
            for(i = 1*3; i <= grid_points[0]-3*1-1; i += 1)
...
#pragma omp parallel for private(j)
         for(m = 0; m <= 4; m += 1)
            for(j = 1*3; j <= grid_points[1]-3*1-1; j += 1)
...
#pragma omp parallel for private(k)
         for(m = 0; m <= 4; m += 1)
            for(k = 1*3; k <= grid_points[2]-3*1-1; k += 1)
...
#pragma omp parallel for private(j, m, k, n)
   for(i = 1; i <= grid_points[0]-1-1; i += 1)
      for(j = 1; j <= grid_points[1]-1-1; j += 1)
